...
> **Note — protocol updated.** The two-WebSocket protocol described below
> (`/session` for control + `/verifier?sessionId=...` for MPC bytes) has been
> replaced by a single `/session` WebSocket that carries both. The rest of
> this document (architecture, webhooks, deployment) is still accurate. For
> the current protocol, see
> [packages/verifier/README.md](packages/verifier/README.md) and
> [CLAUDE.md](CLAUDE.md#verifier-server-package-packagesverifier).
